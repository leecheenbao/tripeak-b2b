...
| 角色 | 姓名 | 學號 | 負責對外事項 |
| --- | --- | --- | --- |
| **計畫主持人（PM）** | 賴叔佑 | 4144W009 | 統一對外窗口：指導教授溝通、課程繳交、外部協作聯繫 |
| **技術副窗口** | 徐仁權 | 4144W008 | 技術規格確認、與外部 API 廠商（LINE）技術對接 |
...
graph TD
    ...
    BE --> BE1["徐仁權（4144W008）<br/>後端 Lead"]
    BE --> BE2["李慶寶（4144W010）<br/>後端工程師"]

    FE --> FE1["吳宇量（4144W012）<br/>前端 Lead"]
    FE --> FE2["劉宥伸（4144W013）<br/>前端工程師"]

    DI --> DI1["林柏諭（4144W014）<br/>資料整合 / DevOps"]
```
...
| 編號 | 姓名 | 學號 | 角色 | 主要職責 |
| --- | --- | --- | --- | --- |
| 1 | **李慶寶** | 4144W010 | 後端工程師 | 身分認證模組（JWT、bcrypt）；訂單 API 開發；Mongoose 資料模型；API 測試（Postman Collection） |
| 2 | **徐仁權** | 4144W008 | 後端 Lead | 身分認證模組（JWT、bcrypt）；API 架構設計；Code Review；與前端 API 契約定義；技術決策紀錄（ADR） |
| 3 | **賴叔佑** | 4144W009 | 計畫主持人（PM） | 專案規劃與進度管控；Sprint 規劃與回顧主持；WBS／甘特圖維護；對外溝通統一窗口；交付物整合審查 |
| 4 | **吳宇量** | 4144W012 | 前端 Lead | Vue 3 / Vuetify 元件規範；路由守衛與 Pinia 架構；前後端整合介面對齊；管理後台頁面 |
| 5 | **劉宥伸** | 4144W013 | 前端工程師 | 經銷商端頁面（產品瀏覽、購物車、結帳、訂單查詢）；Chart.js 報表視覺化；RWD 調整 |
| 6 | **林柏諭** | 4144W014 | 資料整合 / DevOps | MongoDB 綱要設計、索引與種子腳本；LINE Webhook 整合；Docker Compose 環境建置；`.env.example` 與 README 文件化 |

#### RACI 矩陣（核心交付物）

> R = 執行責任　A = 最終負責　C = 諮詢　I = 知會

| 交付物 | 賴叔佑 PM | 徐仁權 後端L | 李慶寶 後端 | 吳宇量 前端L | 劉宥伸 前端 | 林柏諭 DevOps |
| --- | :---: | :---: | :---: | :---: | :---: | :---: |
